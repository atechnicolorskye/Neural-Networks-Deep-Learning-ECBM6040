...
class RNNSLU2(object):
    ...
    def __init__(self, nh, ng, nc, ne, de, cs, normal=True):
        ...
        self.h0 = theano.shared(name='h0',
                                value=numpy.zeros(nh,
                                dtype=theano.config.floatX))
        self.g0 = theano.shared(name='g0',
                                value=numpy.zeros(ng,
                                dtype=theano.config.floatX))


        # bundle
        self.params = [self.emb, self.wx, self.wh, self.whg, self.wg, self.w,
                       self.bh, self.bg, self.b, self.h0, self.g0]
        ...
        def recurrence(x_t, h_tm1, g_tm1):
            ...
            g_t = T.nnet.sigmoid(T.dot(h_t, self.whg)
                                 + T.dot(g_tm1, self.wg) + self.bg)
            s_t = T.nnet.softmax(T.dot(g_t, self.w) + self.b)
            return [h_t, g_t, s_t]

        [h, g, s], _ = theano.scan(fn=recurrence,
                                sequences=x,
                                outputs_info=[self.h0, self.g0, None],
                                n_steps=x.shape[0])
...
def test_rnnslu(**kwargs):
    ...
    print(param['win'], param['nhidden'], param['emb_dimension'], param['normal'])
...
def test_rnnslu2(**kwargs):
    ...
    print(param['win'], param['nhidden1'], param['nhidden2'], param['emb_dimension'])
