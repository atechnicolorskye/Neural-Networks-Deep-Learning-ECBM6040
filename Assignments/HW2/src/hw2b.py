...
def load_data():
    ...
    # valid_set = (x[-250:] for x in train_set)
    # train_set = (x[:750] for x in train_set)
    # test_set = (x[:100] for x in test_set)
...
class myMLP(object):
    def __init__(self, rng, input, n_in, n_hidden, n_out, n_layers, activation):
        ...
        # create list containing n layers
        self.layers = []

        # iterate through range to create n layers
        ...
        # logistic regression
        self.logRegressionLayer = LogisticRegression(
            input=self.layers[n_layers-1].output,
            n_in=n_hidden,
            n_out=n_out)

        # regularise the last 2 layers
        self.L1 = (
            abs(self.layers[n_layers-1].W).sum()
            + abs(self.logRegressionLayer.W).sum()
        )

        self.L2_sqr = (
            (self.layers[n_layers-1].W ** 2).sum()
            + (self.logRegressionLayer.W ** 2).sum()
        )
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
             batch_size=20, n_hidden=500, n_layers=2, activation=T.tanh, verbose=False):
    """
...
if __name__ == '__main__':
    ...
    #          batch_size=20, n_hidden=i, n_layers=3, activation=T.tanh, verbose=True)
    #     print('Tested N Hidden', i)
    ...
    print('N Hidden 2 Layers')
    for i in [1600, 2000]:
        test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=10**-5, n_epochs=1000,
             batch_size=20, n_hidden=i, n_layers=2, activation=T.tanh, verbose=True)
        print('N Hidden 2 Layers', i)

# To Do
# 8 layers
# 1 layer for 4
# 1 Layer 2000, all of 2 layers
